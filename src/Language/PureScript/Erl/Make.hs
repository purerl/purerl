...
buildActions outputDir env foreigns usePrefix =
  ...
  codegen m = do
    ...
    (exports, typeDecls, foreignSpecs, rawErl, safeExports, safeRawErl) <- moduleToErl env m' foreignExports

    optimized <- traverse optimize rawErl
    safe <- traverse optimize safeRawErl
    ...
    let pretty = prettyPrintErl (makeAbsFile :: String -> String) optimized
        prettySafe = prettyPrintErl (makeAbsFile :: String -> String) safe
    ...
    let 
        ...
        directives :: [T.Text] -> ModuleType -> [T.Text]
    ...
    let erlsafe :: T.Text = T.unlines $ map ("% " <>) prefix ++ directives safeExports PureScriptSafeModule ++  [ prettySafe ]
    lift $ writeTextFile (outFileSafe mn) $ TE.encodeUtf8 erlsafe
  ...
  outFileSafe mn = moduleDir mn </> T.unpack (erlModuleName mn PureScriptSafeModule) ++ ".erl"
